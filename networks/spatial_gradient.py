...
def _get_sobel_diag_x_kernel_3x3() -> torch.Tensor:
    return torch.tensor([
        [ 0., 1., 2.],
        [-1., 0., 1.],
        [-2.,-1., 0.],
...
def _get_sobel_diag_y_kernel_3x3() -> torch.Tensor:
    return torch.tensor([
        [-2.,-1., 0.],
        [-1., 0., 1.],
        [ 0., 1., 2.],
...
def _get_laplacian_kernel_3x3() -> torch.Tensor:
    """Utility function that returns a sobel kernel of 3x3"""
    return torch.tensor([
        [ 0., -1.,  0.],
        [-1.,  4., -1.],
...
def _get_second_order_kernel_3x3() -> torch.Tensor:
    """Utility function that returns a sobel kernel of 3x3"""
    return torch.tensor([
        [-1., 2., -1.],
        [-2., 4., -2.],
...
class SpatialGradient(nn.Module):
    ...
    def __init__(self, diagonal=False, laplacian=False, second_order=False) -> None:
        super(SpatialGradient, self).__init__()
        self.kernel: torch.Tensor = self.get_sobel_kernel(diagonal=diagonal, laplacian=laplacian, second_order=second_order)

    @staticmethod
    def get_sobel_kernel(diagonal=False, laplacian=False, second_order=False) -> torch.Tensor:
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:  # type: ignore
        ...
        padding = kernel_flip.size(2) // 2
        return F.conv3d(input[:, :, None], kernel_flip, padding=padding, groups=c)
